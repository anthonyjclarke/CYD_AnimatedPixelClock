/*
 * CYD_AnimatedPixelClock - Ambient: Pac-Man maze
 *
 * Ported from AnimatedPixelClock by Keralots. A self-playing game of Pac-Man
 * that fills the whole canvas (no clock, no digits). The maze is a lattice of
 * pillars with an open perimeter loop, so Pac always has an escape route. Pac
 * plays like a real person: he heads for the nearest pellet when it is safe but
 * swerves away (and reverses) from any ghost that gets close, and he turns to
 * hunt the blue ghosts after eating a power pellet. The four ghosts run classic
 * scatter/chase waves with individual personalities (Blinky chases directly,
 * Pinky aims ahead of Pac, Inky mirrors across Blinky, Clyde only closes in
 * from a distance), so they spread out and corner Pac only rarely instead of
 * dogpiling him. Getting caught costs a life (shrink, then the board resets
 * with the dots intact); clearing every dot lays out a fresh board.
 *
 ...
 *
 * CYD port: cells are 8 sprite pixels, magnified by SPRITE_SCALE (K below),
 * and the maze takes as many odd-sized rows and columns as the canvas holds.
 * The formulas reproduce upstream's 15x7 maze, pen and ghost starts exactly on
 * 128x64: 19x13 on 160x120, 13x9 on 240x160.
 */
...
#define MZ_CELL (8 * K)  // px per cell
// Odd counts: the pillar lattice needs a corridor along every edge.
#define MZ_FIT_COLS ((SCREEN_WIDTH - MZ_CELL) / MZ_CELL)
#define MZ_FIT_ROWS ((SCREEN_HEIGHT - MZ_CELL) / MZ_CELL)
#define MZ_COLS (MZ_FIT_COLS - (1 - MZ_FIT_COLS % 2))
#define MZ_ROWS (MZ_FIT_ROWS - (1 - MZ_FIT_ROWS % 2))
// Origin inset so the maze centers inside a thin outer frame.
#define MZ_OX ((SCREEN_WIDTH - MZ_COLS * MZ_CELL) / 2)
#define MZ_OY ((SCREEN_HEIGHT - MZ_ROWS * MZ_CELL) / 2)
#define GHOST_COUNT 4

static_assert(MZ_COLS >= 7 && MZ_ROWS >= 5, "Pac-Man maze needs at least 7x5 cells");

// Speeds (px/sec at scale 1). Pac keeps a clear edge over the ghosts so a smart
// player can outrun them along the open perimeter.
...
// Pen (ghost home) cell that eaten eyes return to. Odd row and column, so it is
// always corridor, never pillar.
#define PEN_COL ((MZ_COLS / 2) | 1)
#define PEN_ROW ((MZ_ROWS / 2) | 1)
...
static void resetPositions() {
  ...
  const int8_t gc[GHOST_COUNT] = {PEN_COL - 3, PEN_COL - 1, PEN_COL + 1, PEN_COL + 3};
...
// Drawn in sprite pixels around the ghost's centre and magnified from there.
...
void ambientPacmanChaseFrame() {
  ...
  if (deathTimer > 0) {
    ...
    // ---- Pac-Man ----
    float over = advanceActor(pac, SPEED_PAC * K, dt);
    if (over >= 0.0f) {
    ...
    for (int i = 0; i < GHOST_COUNT; i++) {
      ...
      float gover = advanceActor(gh[i], sp * K, dt);
      if (gover >= 0.0f) {
  ...
  // ================= Draw =================
  // Maze: thin outer frame just outside the perimeter corridor (upstream's sat
  // 3 px outside it), one sprite pixel thick, plus the interior pillar lattice.
  for (int t = 0; t < K; t++) {
    display.drawRect(MZ_OX - 3 * K + t, MZ_OY - 3 * K + t,
                     MZ_COLS * MZ_CELL + 6 * K - 2 * t, MZ_ROWS * MZ_CELL + 6 * K - 2 * t,
                     COL_WALL);
  }
  for (int r = 2; r <= MZ_ROWS - 2; r += 2) {
    for (int c = 2; c <= MZ_COLS - 2; c += 2) {
      SpriteScale mag(display, K, ccx(c), ccy(r));
  ...
  for (int r = 0; r < MZ_ROWS; r++) {
    for (int c = 0; c < MZ_COLS; c++) {
      if (dot[r][c]) {
        display.fillRect(ccx(c), ccy(r), K, K, COL_DOT);
  ...
  SpriteScale mag(display, K, (int)pac.x, (int)pac.y);
  if (deathTimer > 0) {
