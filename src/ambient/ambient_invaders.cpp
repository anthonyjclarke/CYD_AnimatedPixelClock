/*
 * CYD_AnimatedPixelClock - Ambient: Space Invaders battle
 *
 * Ported from AnimatedPixelClock by Keralots. Self-playing attract loop (no
 * clock, no digits). An invader fleet marches in classic lockstep - drifting
 * sideways, dropping and reversing at the edges, legs toggling on every step
 * and speeding up as it thins out. An auto-aiming cannon slides under the fleet
 * and fires; invaders drop bombs back. Hits burst into a short starburst. A UFO
 * occasionally crosses the top for a bonus. When the fleet is cleared (or
 * reaches the cannon line) a fresh wave spawns.
 *
 ...
 *
 * CYD port: every sprite is magnified by SPRITE_SCALE (K below), and every
 * distance and speed is in sprite pixels. The fleet is sized to the canvas as
 * upstream's 6x4 was to 128x64 - the formulas give exactly 6x4 there - so a
 * taller, wider canvas gets a bigger fleet rather than a small one in a void.
 */
...
// drop on every step (visibly frozen). Upstream's 75 px of 128 leaves travel
// each way; the column count keeps that share of the width.
#define INV_COL_STEP (15 * K)  // px between invader columns (sprite is 11 wide)
#define INV_ROW_STEP (12 * K)  // px between invader rows
#define INV_FIT_COLS ((SCREEN_WIDTH * 75 / 128) / INV_COL_STEP + 1)
#define INV_FIT_ROWS ((SCREEN_HEIGHT * 36 / 64) / INV_ROW_STEP + 1)
#define INV_COLS (INV_FIT_COLS > 6 ? INV_FIT_COLS : 6)
#define INV_ROWS (INV_FIT_ROWS > 4 ? INV_FIT_ROWS : 4)
#define INV_COUNT (INV_ROWS * INV_COLS)
#define INV_MARGIN (8 * K)  // keep the fleet this far from the side walls
#define INV_STEP_X (3 * K)  // horizontal jump per march step
#define INV_DROP (6 * K)    // vertical drop when reversing
#define INV_TOP_Y (10 * K)  // top row center on a fresh wave
#define CANNON_Y (SCREEN_HEIGHT - 4 * K)  // cannon center row
#define UFO_Y (5 * K)
...
// Screen center of an invader cell.
...
static void marchStep() {
  ...
  int leftEdge = cellX(loCol) - 5 * K + fleetDir * INV_STEP_X;
  int rightEdge = cellX(hiCol) + 5 * K + fleetDir * INV_STEP_X;
...
static void fireCannon() {
  for (int i = 0; i < (int)(sizeof(pBullets) / sizeof(pBullets[0])); i++) {
    if (!pBullets[i].active) {
      pBullets[i] = {cannonX, (float)(CANNON_Y - 3 * K), true};
      return;
    }
  }
}
...
static void dropBomb() {
  ...
  while (tries--) {
    ...
    for (int i = 0; i < (int)(sizeof(bombs) / sizeof(bombs[0])); i++) {
      if (!bombs[i].active) {
        bombs[i] = {(float)cellX(c), (float)(cellY(r) + 8 * K), true};
        return;
      }
    }
    return;
...
static bool hitInvader(float x, float y) {
  for (int r = 0; r < INV_ROWS; r++) {
    for (int c = 0; c < INV_COLS; c++) {
      ...
      if (x >= ix - 6 * K && x <= ix + 6 * K && y >= iy - 5 * K && y <= iy + 9 * K) {
        invAlive[idx] = false;
        invAliveCount--;
        spawnBoom(ix, iy);
        return true;
...
// The small sprites below are drawn in sprite pixels around their centre and
// magnified from there, exactly as upstream drew them at 1:1.
static void drawCannon(int cx, int cy) {
  SpriteScale mag(display, K, cx, cy);
  if (cannonHitTimer > 0) {
...
static void drawBoom(const Boom& b) {
  ...
  SpriteScale mag(display, K, x, y);
  int rad = b.age;  // grows 0..3
  uint16_t col = (b.age < 2) ? COL_BOOM_A : COL_BOOM_B;
  display.drawPixel(x, y, col);
  display.drawPixel(x - rad, y, col);
  display.drawPixel(x + rad, y, col);
  display.drawPixel(x, y - rad, col);
  display.drawPixel(x, y + rad, col);
  display.drawPixel(x - rad, y - rad, col);
  display.drawPixel(x + rad, y + rad, col);
  display.drawPixel(x - rad, y + rad, col);
  display.drawPixel(x + rad, y - rad, col);
}
...
void ambientInvadersFrame() {
  ...
  if (now - lastMarch >= (unsigned long)marchInterval) {
    lastMarch = now;
    marchStep();
  }
  ...
  if (invAliveCount == 0 ||
      cellY(INV_ROWS - 1) + 8 * K >= CANNON_Y - 2 * K) {
    spawnWave();
  }
  ...
  if (cannonHitTimer > 0) {
    ...
    for (auto& b : bombs) {
      ...
      if (b.y < CANNON_Y && fabsf(b.x - cannonX) < 12.0f * K) {
    ...
    if (dodging) {
      ...
      targetX = (threatX > cannonX) ? cannonX - 26 * K : cannonX + 26 * K;
      if (targetX < 6 * K) targetX = cannonX + 26 * K;
      if (targetX > SCREEN_WIDTH - 6 * K) targetX = cannonX - 26 * K;
    } else {
      for (int r = 0; r < INV_ROWS; r++) {
        for (int c = 0; c < INV_COLS; c++) {
  ...
  for (auto& b : pBullets) {
    ...
    if (ufo.active && b.y <= UFO_Y + 2 * K &&
        b.x >= ufo.x - 5 * K && b.x <= ufo.x + 5 * K) {
      spawnBoom(ufo.x, UFO_Y);
      ufo.active = false;
      b.active = false;
      continue;
    }
  ...
  for (auto& b : bombs) {
    ...
    if (cannonHitTimer == 0 && b.y >= CANNON_Y - 2 * K &&
        b.x >= cannonX - 5 * K && b.x <= cannonX + 5 * K) {
  ...
  if (!ufo.active && now >= ufoNextSpawn) {
    ...
    ufo.x = (ufo.dir == 1) ? -6 * K : SCREEN_WIDTH + 6 * K;
    ufoNextSpawn = now + random(9000, 18000);
  }
  if (ufo.active) {
    ufo.x += ufo.dir * 40.0f * K * dt;
    if (ufo.x < -8 * K || ufo.x > SCREEN_WIDTH + 8 * K) ufo.active = false;
  }
  ...
  // ================= Draw =================
  if (ufo.active) drawUfo((int)ufo.x, UFO_Y);

  for (int r = 0; r < INV_ROWS; r++) {
    for (int c = 0; c < INV_COLS; c++) {
      if (!invAlive[r * INV_COLS + c]) continue;
      SpriteScale mag(display, K, cellX(c), cellY(r));
  ...
  for (auto& b : pBullets) {
    if (b.active) display.fillRect((int)b.x, (int)b.y - K, K, 2 * K, COL_PBULLET);
  }
  for (auto& b : bombs) {
    if (b.active) display.fillRect((int)b.x, (int)b.y - K, K, 2 * K, COL_BOMB);
  }
  for (auto& b : booms) {
