/*
 * CYD_AnimatedPixelClock - Doom Fire Clock (clockStyle 17)
 *
 * Ported from AnimatedPixelClock by Keralots, where it was added after v2.3.0.
 *
 ...
 * baseline.
 *
 * ---- CYD port: the fire runs at half resolution ----------------------------
 * Upstream simulated one heat cell per LED on a 128x64 panel. Here one fire cell
 * is a 2x2 block of canvas pixels - an 80x60 grid on the 160x120 canvas, 120x80
 * on 240x160 - for three reasons:
 *
 *   - The heat buffer is 4.8 or 9.6 KB of static RAM. At full resolution it
 *     would be 19-38 KB, which the 4.0" cannot spare beside its canvas.
 *   - Upstream's tuning (64 heat levels crossed in ~20 rows, flame reach of
 *     5-40 rows) was made for a 64-row buffer. 60-80 rows keeps it valid as is,
 *     so the height settings mean fire rows here, as they meant pixels there.
 *   - A digit cell (DIGIT_TEXT_SIZE, 4 or 6 px) is 2 or 3 fire cells - the
 *     latter exactly upstream's 3.
 *
 * The digits are drawn over the fire at full canvas resolution, so the time
 * stays crisp while the flames keep their blocky retro look.
 */
...
// ========== Layout / tuning ==========
#define DOOM_CELL 2                         // canvas pixels per fire cell, each axis
#define DOOM_W (SCREEN_WIDTH / DOOM_CELL)
#define DOOM_H (SCREEN_HEIGHT / DOOM_CELL)
#define DOOM_GC (DIGIT_TEXT_SIZE / DOOM_CELL)  // fire cells per glyph cell

// A digit's heat source has to land on the same pixels as the glyph drawn over
// it, so glyph cells and the digit row must both fall on whole fire cells.
static_assert(DIGIT_TEXT_SIZE % DOOM_CELL == 0,
              "Doom Fire needs DIGIT_TEXT_SIZE to be a multiple of DOOM_CELL");
static_assert(TIME_X % DOOM_CELL == 0,
              "Doom Fire needs the digit row to start on a whole fire cell");

// The classic effect uses 37 heat levels over a tall CRT buffer. Only 60-80
// rows are available here, so the whole ramp is crossed in ~20 of them and 37
// levels would step ~2 palette entries per row - a visible band per row.
...
#define DOOM_GLYPH_ROWS 7        // 5x7 glyph
...
// 5x7 numerals. Drawn from this table rather than the Adafruit GFX font, whose
// numerals differ (its zero is slashed), so each glyph lines up with the heat
// source stamped from the same table. Bit 4 = leftmost column.
...
// heat[y][x] in fire cells, 0 = cold. Static RAM, not heap.
...
// with a modulo) would dominate the simulation cost.
...
// Digit top in canvas pixels, on a whole fire cell. Upstream put it at a
// quarter of the height with the date shown, and centred it without - the same
// proportions on its 64-row panel (16 and 21).
static int doomTimeY() {
  const int glyphH = DOOM_GLYPH_ROWS * DIGIT_TEXT_SIZE;
  const int y = settings.doomShowDate ? SCREEN_HEIGHT / 4 : (SCREEN_HEIGHT - glyphH) / 2;
  return (y / DOOM_CELL) * DOOM_CELL;
}
...
// wanted reach makes both settings read directly as fire rows.
...
// Draw a digit at canvas resolution, one DIGIT_TEXT_SIZE square per glyph cell.
static void doomDrawGlyph(int x0, int y0, uint8_t value, uint16_t color) {
  if (value > 9) return;
  const uint8_t* rows = doomDigitGlyph[value];
  for (int gy = 0; gy < DOOM_GLYPH_ROWS; gy++) {
    for (int gx = 0; gx < 5; gx++) {
      if (doomGlyphLit(rows, gx, gy)) {
        display.fillRect(x0 + gx * DIGIT_TEXT_SIZE, y0 + gy * DIGIT_TEXT_SIZE,
                         DIGIT_TEXT_SIZE, DIGIT_TEXT_SIZE, color);
      }
    }
  }
}
...
// sideways offset, cooling it on the way. `gy` is the digit top in fire rows.
//
...
// over a tall CRT-shaped buffer. Pushing on a short buffer and clearing the
// stale cells instead reads as orange confetti, so this pulls: every
// destination gets exactly one sample, and the sheet of flame stays solid.
static void doomSpread(int gy) {
  ...
  const int softLimit = min(gy + DOOM_GLYPH_ROWS * DOOM_GC + DOOM_GC, doomGroundTop() - 2);
  ...
  for (int y = top; y < DOOM_H; y++) {
    ...
    if (softRow) {
      ...
      // the hard edges off the plume. The floor matters: without it the blur
      // smears sparse embers into a continuous wash on the darkest palette
...
// currently feeding the fire. `gy` is the digit top in fire rows.
static void doomStampSources(const char* glyphs, int gy) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    const int dx0 = DIGIT_X[i] / DOOM_CELL;

    if (i == 2) {
      if (glyphs[2] != ':') continue;
      // Two dots in glyph column 2, rows 2 and 4, each with a one-cell cold halo.
      for (int b = 0; b < 2; b++) {
        const int cx = dx0 + 2 * DOOM_GC;
        const int cy = gy + (2 + 2 * b) * DOOM_GC;
        doomFillHeat(cx - 1, cy - 1, DOOM_GC + 2, DOOM_GC + 2, 0);  // halo
        doomFillHeat(cx, cy, DOOM_GC, DOOM_GC, DOOM_MAX_HEAT);
      }
      continue;
    }
    ...
    for (int cy = 0; cy < DOOM_GLYPH_ROWS; cy++) {
      for (int cx = -1; cx <= 5; cx++) {
        if (doomGlyphLit(rows, cx, cy)) {
          doomFillHeat(dx0 + cx * DOOM_GC, gy + cy * DOOM_GC, DOOM_GC, DOOM_GC, DOOM_MAX_HEAT);
          ...
          doomFillHeat(dx0 + cx * DOOM_GC, gy + cy * DOOM_GC, DOOM_GC, DOOM_GC, 0);
        }
      }
    }
  }
...
// ========== Drawing ==========
// Every canvas pixel is written, cold cells included. Upstream's HUB75 build had
// the render loop skip clearDisplay() for this style, because blanking a buffer
// the panel was still scanning flashed. The CYD canvas is off-screen and only
// reaches the panel in display(), so that clear cannot flash here - it is merely
// redundant, and the render loop is left alone.
static void drawDoomFire() {
  ...
  for (int y = 0; y < DOOM_H; y++) {
    ...
    while (x < DOOM_W) {
      ...
      // Equal-heat runs are common across a row of flame, so one rectangle per
      // run saves thousands of per-cell calls per frame.
      display.fillRect(x * DOOM_CELL, y * DOOM_CELL, run * DOOM_CELL, DOOM_CELL, pal[heat]);
      x += run;
    }
  }
}
...
// doing. A digit mid-burnout is deliberately absent. `gy` is in canvas pixels.
static void drawDoomDigits(const char* glyphs, int gy) {
  for (int i = 0; i < 5; i++) {
    ...
    if (i == 2) {
      display.fillRect(DIGIT_X[2] + 2 * DIGIT_TEXT_SIZE, gy + 2 * DIGIT_TEXT_SIZE,
                       DIGIT_TEXT_SIZE, DIGIT_TEXT_SIZE, color);
      display.fillRect(DIGIT_X[2] + 2 * DIGIT_TEXT_SIZE, gy + 4 * DIGIT_TEXT_SIZE,
                       DIGIT_TEXT_SIZE, DIGIT_TEXT_SIZE, color);
      continue;
    }
...
void displayClockWithDoom() {
  ...
  const int timeY = doomTimeY();         // canvas pixels, for drawing
  const int fireY = timeY / DOOM_CELL;   // fire rows, for the simulation

  doomSpread(fireY);
  doomStampSources(glyphs, fireY);
  drawDoomFire();
  drawDoomDigits(glyphs, timeY);
  ...
  if (settings.doomShowDate) {
    ...
    switch (settings.dateFormat) {
      ...
      default: sprintf(dateStr, "%02d.%02d.%04d", timeinfo.tm_mday, timeinfo.tm_mon + 1, timeinfo.tm_year + 1900); break;
    }
    doomOutlineText(DATE_X, 4, dateStr);
  }
  if (!settings.use24Hour) {
    doomOutlineText(SCREEN_WIDTH - 2 * TEXT1_W - 6, 4, displayed_is_pm ? "PM" : "AM");
  }
